...
| CRITIQUE | Endpoint DELETE sans vérification propriétaire | `post.py:83-123` | Tout utilisateur peut supprimer tout post |
| CRITIQUE | TODO non résolus: `require_role(["any"])` sur routes sensibles | `post.py:90`, `storage.py` | Routes non protégées |
| ÉLEVÉ | Token JWT refresh sans vérification expiration | `auth.py:33-45` | Tokens expirés réutilisables |
| ÉLEVÉ | Pas de validation fichiers (taille, type MIME réel) | `users.py:50-51` | Upload de fichiers malveillants |
